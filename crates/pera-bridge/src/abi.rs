...
impl TryFrom<&TokensDepositedFilter> for EthToPeraTokenBridgeV1 {
    ...
    fn try_from(event: &TokensDepositedFilter) -> BridgeResult<Self> {
        Ok(Self {
            nonce: event.nonce,
            pera_chain_id: BridgeChainId::try_from(event.destination_chain_id).map_err(anyhow::Error::from)?,
            eth_chain_id: BridgeChainId::try_from(event.source_chain_id).map_err(anyhow::Error::from)?,
            pera_address: PeraAddress::from_bytes(event.recipient_address.as_ref()).map_err(anyhow::Error::from)?,
