    ...
    P: PeraClientInner,
{
    ...
    pub async fn query_events_by_module(
        ...
        let events = self.inner.query_events(filter.clone(), cursor).await.map_err(anyhow::Error::from)?;
    ...
    pub async fn get_bridge_action_by_tx_digest_and_event_idx_maybe(
        ...
        let events = self.inner.get_events_by_tx_digest(*tx_digest).await.map_err(anyhow::Error::from)?;
        let event = events
    ...
    pub async fn get_bridge_committee(&self) -> BridgeResult<BridgeCommittee> {
        ...
        for (_, member) in move_type_bridge_committee.members {
            ...
            let pubkey = BridgeAuthorityPublicKey::from_bytes(&bridge_pubkey_bytes).map_err(anyhow::Error::from)?;
    ...
    pub async fn get_chain_identifier(&self) -> BridgeResult<String> {
        Ok(self.inner.get_chain_identifier().await.map_err(anyhow::Error::from)?)
    }
...
impl PeraClientInner for PeraSdkClient {
    ...
    async fn get_token_transfer_action_onchain_status(
        ...
        .await
        .and_then(|status_byte| BridgeActionStatus::try_from(status_byte).map_err(anyhow::Error::from).map_err(Into::into))
    }
