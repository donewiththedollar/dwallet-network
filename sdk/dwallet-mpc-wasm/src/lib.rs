...
pub fn create_sign_centralized_output(centralized_party_dkg_output: Vec<u8>, presign_first_round_output: Vec<u8>, presign_second_round_output: Vec<u8>, message: Vec<u8>, hash: u8, session_id: String) -> Result<JsValue, JsErr> {
    let (sign_message, centralized_output, presign, hash_msg) = create_sign_output(centralized_party_dkg_output, presign_first_round_output, presign_second_round_output, message, hash, session_id).map_err(to_js_err)?;
    Ok(serde_wasm_bindgen::to_value(&(sign_message, centralized_output, presign, hash_msg)).unwrap())
}
