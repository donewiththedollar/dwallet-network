...
exports.submitDWalletCreationProof = submitDWalletCreationProof;
exports.submitTxStateProof = submitTxStateProof;
...
async function submitDWalletCreationProof(dwallet_client, sui_client, configObjectId, dWalletCapId, txId, serviceUrl, keypair) {
    ...
    let txb = new index_js_1.TransactionBlock();
...
async function submitTxStateProof(dwallet_client, sui_client, configObjectId, capWrapperRef, signMessagesId, txId, serviceUrl, keypair) {
    let tx = await sui_client.getTransactionBlock({
        digest: txId,
        options: {},
    });
    ...
    const signSessionRef = result.effects?.created?.filter((o) => o.owner === 'Immutable')[0]
        .reference;
    const signOutput = await (0, utils_js_1.fetchObjectBySessionId)(signSessionRef.objectId, `${packageId}::${dWalletModuleName}::SignOutput`, keypair, dwallet_client);
...
async function retrieveEpochCommitteeIdByEpoch(client, targetEpoch) {
    const query = {
        MoveModule: {
